...
## Current version: V4 — Confession Corridor + Places navigation (over V2's SpacetimeDB backend)
...
| `src/wall.ts` | Wall page: toolbar (**Draw·Text·Erase·React** + More), top-bar, redesigned `.oi-*` popup, 🎲 discover FAB + reveal, **quick-react palette**, `maybeShowDrawHint`, profile panel, onboarding, share/copy-link, **Places dropdown + confession tool/replies/hide (V4)** |
| `src/seed.ts` | Seeded content: `buildSeedObjects()` (33-piece narrative cast) + `buildSeedPayloads()` for the server `seedObjects` reducer; **corridor seeds + `buildConfessionSeedPayloads` (V4)**. `seedWallIfNeeded()` (client/localStorage seeding) only runs when `!isSpacetime()` |
| `src/storage.ts` | `StorageBackend` interface + `LocalStorageBackend` implementation + `activeBackend` module state (`setBackend`/`getBackend`/`isSpacetime`) + convenience helpers that delegate to whichever backend is active; **`hideObject`/`hideComment` facades (V4)** |
| `src/spacetime.ts` | `SpacetimeBackend` — the active backend. Connects to SpacetimeDB, subscribes to tables, converts rows ↔ `WallObject`, calls reducers for every mutation, reconnects with backoff; **hidden-row filtering, corridor seeding, hide impls (V4)** |
...
```ts
{
  id, type ('stroke'|'text'|'shape'|'sticker'|'image'|'secret'|'timecapsule'|'confession'),
...
- `#/wall?place=confession` hardcoded place (V4): instant `teleportTo(12000, 12000, 0.8)` on load — the only deep link that arrives instantly (`teleportTo(..., instant)`), because rAF-driven tweens can freeze in background/occluded tabs.
...
## V4 — Confession Corridor + Places navigation

Request: a far-away themed region ("Confession Corridor") reachable from a **hardcoded Places dropdown** in the top bar, a **landing-page CTA**, **5–8 seeded confessions**, marks that **never fade**, replies framed as "replies" not comments, a **one-click server-side hide** (given the no-moderation floor), and all conflict rules decided up-front. Scope strictly to Features 1–4 of the brief; per-identity caps, OS notifications, chat/communities, and DB-backed Places are all non-goals.

### Decisions locked in before building
- **New `confession` object type** (string `type` column + `VALID_OBJECT_TYPES` whitelist), not a reuse of `secret`. Secrets have reveal/dashed-circle/hidden-preview semantics that would fight a confession's need to be directly readable and replyable. A dedicated type also gives clean per-type reaction pools and preview copy with zero branching on other types.
- **keptForever confessions** (no `expiresAt`). Tradeoff accepted: the corridor saturates over time. If that becomes the problem, the hide floor already exists and a future cap is the lever — but per-identity caps were explicitly a non-goal this round.
- **Corridor center (12000, 12000)** — far from every seed cluster (ancient at 9800,−4200; 404 at 4300,2900). 8 seeds placed within ±360 of center; the Places camera (zoom 0.8, hand-tuned after seeding) frames the whole room.
- **Server-side global hide, scoped to corridor content only** — `hideObject({id})` / `hideComment({id})` reducers reject `type !== 'confession'`. Anyone may hide (the floor is "get it off the wall", not a moderation system); documented as the MVP floor. This is a deliberate asymmetry: a flag column (`hidden`, default false) added to `canvas_object` and `comment` lets the server stop streaming a row without deleting data.
- **Hidden rows are filtered client-side** (`onCanvasUpsert`/`onCommentUpsert`), so no client ever renders a hidden mark/reply — no subscription-query rewrite, and stale copies from earlier syncs are cleared in place.

### Work log
- **Module** (`spacetimedb/src/index.ts`): `'confession'` in `VALID_OBJECT_TYPES`; `hidden: t.bool().default(false)` on `canvas_object` + `comment` (additive + defaulted → migration-safe hot-swap republish; `.default()` also blocks the update-hazard on old rows); `applySeeds` refactored so `seedObjects` (seed_state id 1) and the new `seedConfessions` (id 2) share one guard — already-seeded databases (local AND maincloud) get corridor content on the next client connect without re-running main-wall seeds; `hideObject`/`hideComment` reducers. Regenerated `src/module_bindings/` (`spacetime generate`, no server needed).
- **Client**: `types.ts` (`'confession'` in the union, `Comment.id`), `seed.ts` (8 authored confessions — mother, cooking-for-two, tired, 2 a.m., gentle replies, the job, book margins, "stay. one more day." — + `buildConfessionSeedPayloads`/`toPayload` refactor), `canvas.ts` (tool `confession`, `placeConfession` soft-lavender overlay, render case, `drawGrid` now paints a **candlelit "room"** around the corridor — radial floor + dashed border + world-space title, all `listening:false`, world-anchored so it pans/zooms with the wall — and `teleportRandom` excludes confessions), `wall.ts` (🕯️ tool, `PLACES` dropdown, `place` deep-link, per-type reaction pools `CONFESSION_REACTIONS = ['🕯️','❤️','🫂','💛','🙏','✨']`, confession popup gets a primary **💬 Reply** + **🚩 Hide**, replies modal framed as "💬 Replies / leave a reply..." with per-reply 🚩, `doWeirdDiscovery` excludes confessions), `storage.ts`/`spacetime.ts` (hide facades + hidden filtering + corridor seeding on first connect), `main.ts` (`place` route param), `landing.ts` (🕯️ CTA), `style.css` (`.btn-tertiary`, `.places-menu`, `.oi-action.danger`, `.comment-hide`).
- **Real bug found during verification**: load-time teleports animate with Konva `stage.to()`, which is requestAnimationFrame-driven. In an occluded/background tab rAF is frozen and the tween never completes — a `#/wall?place=confession` deep link opened in a background tab would never actually arrive. Fix: `teleportTo(x,y,zoom,instant)` — the deep-link/place path passes `instant=true` (transform applied synchronously), and `document.hidden` also forces the instant path; the smooth 0.8s animation remains for in-app navigations (Places dropdown) where the tab is by definition in the foreground.

### Verification
- **Local** (own SpacetimeDB server, module republished, two-browser flow): landing CTA → instant hop to the corridor (camera (12000,12000) @ 0.8) with all 8 seeds in view; confession popup = 🕯️🫂 palette + kept-forever ∞ + 💬 Reply + 🚩 Hide; reaction persists; reply framed "💬 Replies / leave a reply..."; reply hidden via per-row 🚩 and gone from store + UI; 🕯️ tool writes a kept-forever confession under the cursor; 🚩 Hide removes a mark everywhere (server row hidden + local cache cleared); 12× `teleportRandom` never picks a confession; corridor room (floor/border/title) present on the grid layer.
- **Prod** (maincloud ghostwall published, additive-only migration, `vercel --prod`): CTA live, deep link lands at zoom 80%, viewport persisted to the corridor coords, connection live (online count), Places dropdown lists both places.
...
- [x] Confession Corridor + Places navigation (V4, see above) — code, local + prod verification, maincloud module republished, `vercel --prod` deployed
- [ ] Commit + push to GitHub (nothing on GitHub yet; the site deploys from `npx vercel --prod`, not from a push-triggered build)
...
- [ ] Confession Corridor saturation — confessions never expire, so the corridor fills up over time by design; the 🚩 hide floor exists, but if the corridor gets cluttered the next lever is the deliberately-deferred per-identity cap (see V4)